...
- Coolify 已建立 `futuremint-ai-postgres`、`futuremint-ai-api`、`futuremint-ai-web`；目前只有 PostgreSQL running。API／Web 尚未首次部署；正式 domains、runtime secrets、`ALLOWED_ORIGINS` 與前端 `API_BASE_URL` 已設定，等待首次 production deployment 驗證。
