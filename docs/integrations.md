...
| GitHub | 目標為 private repository `main` 自動部署 | 本機 repository 與 remote 存在 | 尚未連 Coolify 或驗證 Auto Deploy |
...
不整合支付、銀行、電子發票、證券下單、Apple Pay、LINE Pay、Email、SMS 或真實未成年人金融服務。TWSE OpenAPI 只提供公開延遲行情，不會建立真實證券帳戶或交易。主辦方 Azure 關閉後，runtime 也不再依賴任何 Azure service。
