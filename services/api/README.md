...
FutureMint AI 的可信任後端，提供 Azure Functions v4 HTTP API、帳號／session、Zod 契約、確定性財務計算、Azure OpenAI／Demo AI providers，以及 Cosmos DB／Memory repositories。
...
登入路由（需要 `Authorization: Bearer <opaque-session-token>`）：
...
所有回應都有去識別 `requestId`；錯誤使用穩定 `code`、繁中 `message`、`retryable` 與可選 `fieldErrors`，不回傳 stack、prompt、password hash、session token 或環境值。`GET /api/health` 只檢查 runtime 設定與進程存活，不能當成雲端依賴已通過的 readiness 證據。

## 帳號與資料邊界

- 電子郵件正規化後唯一；密碼至少 12 個字元，必須包含英文字母與數字。
- 密碼以每帳號 16-byte salt 的 `scrypt` 雜湊保存，不保存明文。
- 登入 token 是 32-byte opaque random token；只保存 SHA-256 hash，7 天後到期，登出會撤銷 session。
- API 從 Bearer token 取得帳號主體；任何 profile、event、lesson 讀寫都不接受 Client 提供的 `userId` 作為權限依據。
- 此競賽原型尚未有 email 驗證、忘記密碼、帳號刪除、session 管理、rate limit、年齡／監護同意或 production incident response。
...
本機 Web 若使用 `--web-port=4173`，將 `ALLOWED_ORIGINS` 設為 `http://localhost:4173`。多個 origin 以逗號分隔；未在名單中的 preflight 會回覆 403，不使用任意 `*`。CORS 允許 `authorization` header。

Cosmos adapter 不會自動建立雲端資源。需人工建立 `accounts`、`sessions`、`profiles`、`moneyEvents`、`learning` containers，partition key 都是 `/userId`；session 查詢以 token hash 查找後再讀帳號主體。

## 受控 Cosmos 合成種子

Repository 提供可重複執行的 `npm run seed:cosmos-demo`，只會寫入固定 `demo-user` 合成 profile 與四筆帶 idempotency key 的合成事件。它不會建立 Azure 資源或 containers，也不會建立可登入帳號或刪除資料。
